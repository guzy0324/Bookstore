...
from app.test import SQL_Server
...
def role():
    ...
    if not sql.ifManager(username, password):
        ...
        return render_template('user.html', user=username, tableList=tableList, attributeList=attributeList,
...
@app.route('/developer/update', methods=['GET', 'POST'])
def updatePanel():
    """detect the table which is to be inserted"""
    global table
    table = request.form['table']
    ...
    if len(results) != 0:
        ...
        col = 0
    return render_template('developer_update.html', table=table, attributeList=attributeList, results=results, attribute=attribute, row=len(results), col=col, attributeLen=len(attributeList))

@app.route('/developer/update/result', methods=['GET', 'POST'])
def updateResult():
    """show result after insertion"""
    global table
    attribute = '*'
    ...
    if len(results) != 0:
        ...
        col = 0
    return render_template('developer_update_result.html', table=table, attributeList=attributeList, results=results, attribute=attribute, row=len(results), col=col, SF=SF)
...
def userPanel():
    ...
    tableList = sql.tableList
    attributeList = sql.attributeList
    return render_template('user.html', user=username, tableList=tableList, attributeList=attributeList, tableLen=len(tableList))
...
def userselectPanel():
    ...
    if not sql.tableDetect(table):
        ...
        return redirect('/developer')
    attributeList = sql.getAttributeListOfTable(table)
    return render_template('user_select.html', table=table, attributeList=attributeList, attributeLen=len(attributeList))

@app.route('/user/select/result', methods=['GET', 'POST'])
def userSelectResult():
    ...
    if len(results) != 0:
        ...
        col = 0
    return render_template('user_select_result.html', table=table, attributeList=attributeList, results=results, attribute=attribute, row=len(results), col=col)
...
def userInsertPanel():
    ...
    if len(results) != 0:
        ...
        col = 0
    return render_template('user_insert.html', table=table, attributeList=attributeList, results=results, attribute=attribute, row=len(results), col=col, attributeLen=len(attributeList))

@app.route('/user/insert/result', methods=['GET', 'POST'])
def userInsertResult():
    ...
    if len(results) != 0:
        ...
        col = 0
    return render_template('user_insert_result.html', table=table, attributeList=attributeList, results=results, attribute=attribute, row=len(results), col=col, SF=SF)
